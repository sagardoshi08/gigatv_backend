...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.liveShowFeed.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.live-show-feeds.update", [$liveShowFeed->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="show_title">{{ trans('cruds.liveShowFeed.fields.show_title') }}</label>
                <input class="form-control {{ $errors->has('show_title') ? 'is-invalid' : '' }}" type="text" name="show_title" id="show_title" value="{{ old('show_title', $liveShowFeed->show_title) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\LiveShowFeed::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $liveShowFeed->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('day') ? 'is-invalid' : '' }}" name="day" id="day" required>
                    ...
                    @foreach(App\Models\LiveShowFeed::DAY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('day', $liveShowFeed->day) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('day'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('start_time') ? 'is-invalid' : '' }}" type="text" name="start_time" id="start_time" value="{{ old('start_time', $liveShowFeed->start_time) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('end_time') ? 'is-invalid' : '' }}" type="text" name="end_time" id="end_time" value="{{ old('end_time', $liveShowFeed->end_time) }}">
